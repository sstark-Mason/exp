<script lang="ts">
  ...
  import { newDbKey, updateDb, getSupabase, signOutDbKey } from "$lib/db/db_ccg_client";
  ...
  async function injectSampleData() {
    ...
    let db_uid: string | null = await newDbKey(`sample_`);
    let avatar_index = 0;
    ...
    for (let i = 0; i < 300; i++) {
      if (i % 30 === 0) {
        ...
        db_uid = await newDbKey(`sample_${i/30}`);
        avatar_index++;
        if (!db_uid) {
