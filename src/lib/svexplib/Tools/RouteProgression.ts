...
export function getRoutePermission(route: string): PermissionState {
  ...
  switch (routeEntry?.state) {
    ...
    default:
      debug(
        `Unknown permission state for route ${route}. Defaulting to NotYetPermitted.`,
      );
      return PermissionState.NotYetPermitted;
  }
}
...
export function findNextPermittedRoute(
  currentRoute: string,
  routeOrder: string[],
): string | null {
  const expProgress = getProgress();
  ...
  for (let i = currentIndex + 1; i < routeOrder.length; i++) {
    ...
    const routeState = routeEntry
      ? routeEntry.state
      : PermissionState.NotYetPermitted;
    switch (routeState) {
      case PermissionState.NotYetPermitted:
        return nextRoute;
      case PermissionState.Permitted:
...
export function gotoNextPermittedRoute(
  currentRoute: string,
  routeOrder: string[],
): void {
  const expProgress = getProgress();
  ...
  debug(
    `Current route: ${currentRoute}, index: ${currentIndex}. ExpProgress: ${
      JSON.stringify(expProgress)
    } (length: ${expProgress.length})`,
  );
  ...
  debug(
    `No permitted route found to navigate to from current route: ${currentRoute}`,
  );
}
